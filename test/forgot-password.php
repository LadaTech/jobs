...
$pass=MD5($password);
$email_check=$db->prepare("select * from job_seeker where Email_id='$username'");
$email_check->execute();
...
$result=$db->prepare("UPDATE  job_seeker set Password='$pass' where Email_id='$username'");
$result->execute();
...
$p_a=$my_path."/login/password-sent.aspx";
header("Location: $p_a");  
}
else {
...
<div class="col-md-4 col-sm-6 login_form">
   <form name="submenus" action="<?php echo $my_path; ?>/forgot-password.aspx" method="post" class="form-horizontal" id="identicalForm" enctype="multipart/form-data"> 
...
<p class="new_Account">For Login? <a href="<?php echo $my_path; ?>/login.aspx">Back to login</a></p>
</div>
<div class="col-lg-4">
<!-- open1 is given in the class that is binded with the click event -->
