...
<?php
$sql_dom="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname FROM chat_info i where jid='$user_info[Job_Seeker_Id]' order by id desc";
...
?>    
            <tr class="<?php if($row_dom["jsread"]==0) { echo "unread"; }?>">
                <td><?php echo $row_dom["cwname"]; ?></td>
                <td><?php echo substr($row_dom["msg"],0,150); ?></td>
                <td><?php echo date("d M Y h:i A",strtotime($row_dom["created_on"])); ?></td><td><a href="<?php echo $my_path; ?>/job-seeker/cw-<?php echo $row_dom["cwid"]; ?>/start-chat.aspx" class="btn-rsms"><i class="fa fa-mail-forward"></i> Chat</a></td></tr>
<?php } ?> 
        </tbody>
    </table>
<?php } ?>
