...
/**********cw order info**************/
$cwoid=$db->query("select * from cw_ordernow where id=$_GET[cwoid]");
if($cwoid->rowCount()==0){
$p_a=$my_path."/cw/dashboard.aspx";
header("Location: $p_a");  
}
$cwo_info=$cwoid->fetch(PDO::FETCH_ASSOC); 
/**********cw order info**************/
/***************** JSD Info **********************/
$jsd=$db->query("SELECT * FROM job_seeker where Job_Seeker_Id=$cwo_info[jid]");
if($jsd->rowCount()==0){
$p_a=$my_path."/cw/dashboard.aspx";
header("Location: $p_a");  
}
$jsd_info=$jsd->fetch(PDO::FETCH_ASSOC); 
/***************** JSD Info ********************/
$templates=$db->query("select * from templates where id=$cwo_info[selected_template]");
...
$stmtje=$db->prepare("SELECT * FROM job_seeker WHERE Job_Seeker_Id ='".$jsd_info['Job_Seeker_Id']."'");
...
$stmtwe=$db->prepare("SELECT * FROM js_work_experience WHERE Job_Seeker_Id ='".$jsd_info['Job_Seeker_Id']."' order by js_work_experience_id desc limit 0,1");
...
?>
<?php
$sql="SELECT * FROM job_seeker WHERE Job_Seeker_Id ='".$jsd_info['Job_Seeker_Id']."'";
//echo $sql;
?>
...
<!--            <button class="btn  toolbar btn btn-primary open2 pull-right">Continue</button>-->
    ...
    <button class="btn  toolbar btn btn-primary open2 pull-right" id="continue_resume">Complete</button>
        ...
        <form method="post" id="save_resume_form" action="<?php echo $my_path;?>/cw/save-resume.aspx" >
            <input type="hidden" name="cwoid" value="<?php echo $_GET["cwoid"]; ?>" />
            <input type="hidden" name="jsid" value="<?php echo $cwo_info["jid"]; ?>" />
            <input type="hidden" name="selected_template" value="<?php echo $cwo_info["iid"]; ?>" />
            <input type="hidden" name="cwid" value="<?php echo $cwo_info["cwid"]; ?>" />
...
<?php include "templates/cw_Viewer.php"; ?>
...
//?>
			var defaultJson = {
            contact: {
                html: '<?php echo $jsd_info['Email_id']; ?><br/><?php echo $jsd_info['Phone_No']; ?>'
            },
            blocks: [{
                category: 'text',
                title: 'Summary',
                html: '<?php echo $jsd_info["Objective"]; ?>'
            } 
		
<?php
$lang=$db->query("select * from js_work_experience where Job_Seeker_Id='$jsd_info[Job_Seeker_Id]'");
if($lang->rowCount()!=0){
?> ,{
                category: 'experience',
...
<?php
$lang=$db->query("select * from js_educational_information where job_seeker_id='$jsd_info[Job_Seeker_Id]'");
if($lang->rowCount()!=0){
?>				
            ,{
                category: 'education',
 ...
 //_t.save = function() {
            // function() {  
             ...
             //  alert();
                ...
                json.person.firstName = "<?php echo  $jsd_info["First_name"]; ?>";
                json.person.lastName = "<?php echo  $jsd_info["Last_name"]; ?>";
                json.person.jobTitle = "<?php echo  $jsw['Designation']; ?>";
                json.person.location = "<?php echo  $jsd_info["Address"]; ?>";
                var resume = {
