...
<li class="active">Manage Invitations</li>
</ol>
</section>
...
<div class="input-group">
    <a href="manage_invitations.php" class="btn btn-success">View All</a>
</div>
</div>
<?php } ?>
</div>
...
<tbody>
<?php
$loop_page='invitations';
if(isset($_GET['ad_cat']))
{
$q=$_GET['ad_cat'];
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname FROM  invitations i where created_on like '%$q%'  order by  id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
else {
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname FROM  invitations i  order by id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
?>																			
...
$r_url="manage_invitations.php?ad_cat=$_GET[ad_cat]";
$obj->page1($sql1,$no,$r_url,$db);
}
else  {
  include 'pagination.php';
  $obj->page($sql1,$no,"manage_invitations",$db);
}
?>
</ul>
</div>
