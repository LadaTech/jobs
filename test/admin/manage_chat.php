...
<div class="input-group">
    <a href="manage_chat.php" class="btn btn-success">View All</a>
</div>
</div>
<?php } ?>
</div>
...
{
$q=$_GET['ad_cat'];
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname FROM  chat_info i where created_on like '%$q%'  order by  id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
else {
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname FROM  chat_info i  order by id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
?>
...
$r_url="manage_chat.php?ad_cat=$_GET[ad_cat]";
$obj->page1($sql1,$no,$r_url,$db);
}
else  {
  include 'pagination.php';
  $obj->page($sql1,$no,"manage_chat",$db);
}
?>
</ul>
</div>
