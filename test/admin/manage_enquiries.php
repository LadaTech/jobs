...
<tr>
        <td width="50">
<input type="text" class="form-control date" autocomplete="off" placeholder="Search by  date" name="ad_cat" style="margin-right: 15px" value="<?php if(isset($_GET['ad_cat'])) { echo $_GET['ad_cat']; }?>" id="post_cat" required="required">
        </td>
...
<div class="input-group">
    <a href="manage_enquiries.php" class="btn btn-success">View All</a>
</div>
</div>
<?php } ?>
</div>
...
{
$q=$_GET['ad_cat'];
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname, (select name from  industry where id=i.iid) iname, (select name from domains where id=i.did) dname FROM enquiry_info i  where created_on like '%$q%'  order by  id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
else {
$sql1="SELECT *,(select First_name from content_writer where Content_writer_id=i.cwid) as cwname,(select First_name from job_seeker where Job_Seeker_Id=i.jid) as jname, (select name from  industry where id=i.iid) iname, (select name from domains where id=i.did) dname FROM enquiry_info i  order by id desc";
$rs= $db->query("$sql1 limit $id,$no") or die(mysql_error());
include "loop_hub.php";
}
?>																			
...
$r_url="manage_enquiries.php?ad_cat=$_GET[ad_cat]";
$obj->page1($sql1,$no,$r_url,$db);
}
else  {
  include 'pagination.php';
  $obj->page($sql1,$no,"manage_enquiries",$db);
}
?>
</ul>
</div>
