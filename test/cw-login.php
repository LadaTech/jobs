...
<h4>	<i class="icon fa fa-check"></i> Congratulations, You have successfully Registered. Please login below.</h4>
...
<div class="alert alert-danger alert-dismissable">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
<h4><i class="icon fa fa-times"></i> Sorry! Account not yet activated. Open your registered email account and  verify your email address <a href="<?php echo $my_path; ?>/cw/send-activation-link.aspx"  class="resend-link">Resend activation link</a></h4>
...
<div class="alert alert-danger alert-dismissable">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
<h4><i class="icon fa fa-times"></i> Sorry! Your account has to be verified and approved by Jatka.in. Confirmation will be sent to your email soon...</h4>
...
$c=$db->query("SELECT Content_writer_id FROM content_writer WHERE email_verification='$activation'");

if($c->rowCount() > 0)
{
$c1=$db->query("SELECT Content_writer_id FROM content_writer WHERE email_verification='$activation' and email_verification_status=0");

if($c1->rowCount() == 1)
{
$db->query("UPDATE content_writer SET email_verification_status='1' WHERE email_verification='$activation'");
?>
<div class="alert alert-success alert-dismissable">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
...
<div class="col-md-4 col-sm-6 login_form">
   <form name="submenus" action="<?php echo $my_path; ?>/cw/login.aspx" method="post" class="form-horizontal" id="identicalForm" enctype="multipart/form-data"> 
...
<div class="col-lg-12">
    ...
    <a href="<?php echo $my_path; ?>/cw/forgot-password.aspx" style="float:right;">Forgot Password?</a>
...
<p class="new_Account">New User? <a href="<?php echo $my_path; ?>/cw/register.aspx">Register</a> today.</p>
</div>
<div class="col-lg-4">
<!-- open1 is given in the class that is binded with the click event -->
...
try {
$stmtjs=$db->prepare("SELECT * FROM content_writer WHERE Email_id = :u && Password = :p");
...
echo $p_a=$my_path."/cw/$clean.aspx";
$t_date=date("d F Y");
unset($_SESSION["cwemail"]);
$db->query("UPDATE content_writer SET online='1' WHERE Content_writer_id='$js[Content_writer_id]'");
$_SESSION["cwid"]=$js['Content_writer_id'];
//exit();
header("Location: $p_a");        
}
else{
if($js['email_verification_status']=='0') { 
$_SESSION["cwemail"]=$username;    
$p_a=$my_path."/cw/login/not-activated.aspx";
header("Location: $p_a");           
}  
else {
if($js['status']=='0') { 
    $p_a=$my_path."/cw/login/account-deactivated.aspx";
    header("Location: $p_a");           
   }
}
}
 ...
 $p_a=$my_path."/cw/login/not-valid.aspx"; // not valid details
header("Location: $p_a");       
}
}
...
var a=$(this).val();
if(a=="1"){
window.location.href="<?php echo $my_path; ?>/login.aspx";
}
});
</script>  