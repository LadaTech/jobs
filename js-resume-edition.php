...
$sr=$db->query("SELECT * FROM js_my_resumes where jsid='$user_info[Job_Seeker_Id]' and id=$_GET[id]");
if($sr->rowCount()==0){
$p_a=$my_path."/job-seeker/dashboard.aspx";
header("Location: $p_a");  
}
$selected_resumes=$sr->fetch(PDO::FETCH_ASSOC);
$templates=$db->query("select * from templates where iid in(select iid from templates where id=$selected_resumes[selected_template])");
...
<form action="<?php echo $paypal_url; ?>" method="post" class="display-inline">
        ...
        <input type="hidden" name="item_name" value="test resume<?php //echo $event["event_name"]; ?>">
        <input type="hidden" name="item_number" value="1221<?php //echo $event["eid"]; ?>">
        <input type="hidden" name="amount" value="10<?php //echo $event["fee"]; ?>">
...
<!--       <input type="submit" name="submit" class="btn  toolbar btn btn-primary open2 pull-right" value="Payment Now"/>-->
    <button class="btn  toolbar btn btn-primary open2 pull-right">Continue</button>
            ...
            <button class="btn  toolbar btn btn-primary open2 pull-right" id="save_resume">Update</button>
            
          </div>
        </div>
        <div class="clearfix"></div>
        <form method="post" id="save_resume_form" action="<?php echo $my_path;?>/job-seeker/update-resume.aspx" >
            <input type="hidden" name="selected_template" value="<?php echo $selected_resumes['selected_template']; ?>" />
            <input type="hidden" name="myrid" value="<?php echo $_GET['id']; ?>" />
            <textarea name="resume_text" class="hidden" id="resume_text"></textarea> 
            <textarea name="html_data" class="hidden1" id="html_data"></textarea> 
            <textarea name="css_data" class="hidden1" id="css_data"></textarea> 
        </form>    
        ...
        <div class="papersheet-outer" id="resume" >
            <div id="resume-papersheet-inner" class="papersheet-inner">
...
<?php include "templates/edit_Viewer.php"; ?>
  ...
  $("#save_resume").click(function(){
           // alert($("#resume-papersheet-inner").html());
            $("#resume_text").val($("#resume-papersheet-inner").html());
               alert(JSON.stringify(defaultJson));
