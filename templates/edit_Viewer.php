...
Viewer.a13 = function(viewer, template) {
    ...
    _t.render = function(data, a44) {
        ...
        $p.children().remove();
        if (data) {
            $("#css_data").html(_t.a14(true));
            $p.html('<style>' + _t.a14(true) + '</style>' + _html(data));
         //   $p.html('<style><?php echo $selected_resumes["css_data"]; ?></style><?php echo $selected_resumes["html_data"]; ?>');
...
<?php if($selected_resumes['selected_template']==$tepl2["id"]) { ?>
