...
?>    
   <form name="submenus" action="<?php echo $my_path; ?>/change-password.aspx" method="post" class="form-horizontal" id="identicalForm" enctype="multipart/form-data" onsubmit="return validateform();"> 
...
try{    
$stmt=$db->prepare("update job_seeker set Password=:Password where Job_Seeker_Id=:Job_Seeker_Id");
$stmt->bindParam(':Password', $password, PDO::PARAM_STR);
$stmt->bindParam(':Job_Seeker_Id', $_SESSION["uid"], PDO::PARAM_STR);
$stmt->execute(); 
if($stmt->rowCount()>0)
{
    
    $pa=$my_path."/password-updated.aspx";
    header("Location: $pa");
}
 else {
     $pa=$my_path."/password-updated.aspx";
     ...
     $pa=$my_path."/wrong-password.aspx";
     header("Location: $pa");
 }
}
ob_end_flush();
